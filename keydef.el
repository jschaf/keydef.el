...
(defvar mode-map-alist
  ...
  "An alist mapping a mode name to its mode map.
If the local keymap for foo-mode is bar-mode-map instead of
foo-mode-map, this alist allows you to specify what corresponds
to what.  The car of each pair should be a major mode name and the
cdr should be the name of the local map that is used for that
mode.")
...
(defun keydef-lib-lookup (mode)
  "Try to figure out where MODE is located.
For a not-already-loaded MODE function, try to determine what
library it would be loaded from: First check for autoload
information, otherwise check if a library file matching the MODE
name can be found in the load path, with or without the -mode
suffix.  Failing that, give up."
...
CMD is automatically wrapped in an anonymous interactive function
if it is Emacs Lisp code rather than a command name. SEQ may also
have the form \(MODE SEQ\) where the car is a mode name\; for
example

  \(keydef \(latex \"C-c %\"\) comment-region\)

means to define the given key in latex-mode-map. And this will
work even if latex-mode is not loaded yet, provided that it is
possible to deduce the file that it will be loaded from, either
from the autoload info or by searching for a matching file name
in the Emacs load path.

Or it may take the form \(MODE SEQ INIT\) where INIT is mode-hook
which will initialize the keys every time the mode-hook runs.

For best results, the \"mode name\" that you use here should
yield the proper foo-mode-map symbol when \"-mode-map\" is
appended\; although this will normally match the mode name as
given in the mode line, Shell-script is one example I can think
  ...
  (let ((map (quote global-map))
    ...
    (if (and (not loaded) modestring)
        (let ((loadfrom (keydef-lib-lookup modestring)))
          (if loadfrom
              (cond ((eq init 'mode-hook)
                     `(eval-after-load ,loadfrom
                        (quote (add-hook (quote ,loadfrom)
            ...
            (message "keydef: '%s' unknown %s"
                     ...
                     "\(perhaps misspelled, or needs autoload info?\)")))
      (cond ((eq init 'mode-hook)
             `(quote (add-hook (intern ,modestring)
                               (quote (define-key ,map (kbd ,seq) ,cmd)))))
            (t
